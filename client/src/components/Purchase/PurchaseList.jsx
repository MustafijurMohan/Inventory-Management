...
import { PurchaseListRequest } from '../../api/PurchaseApiRequest';
...
const PurchaseList = () => {
  ...
  const handlePageClick = async (e) => {
    ...
    await PurchaseListRequest(pageNo + 1, perPage, searchKeyword)
  }
                          ...
                          <tbody>
                            {
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    <td><p className="text-xs text-start">{item.supplier[0]['Name']}</p></td>
