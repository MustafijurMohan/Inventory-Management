...
import { BrandDeleteRequest, BrandListRequest } from '../../api/BrandApiRequest';
...
const BrandList = () => {
  ...
  const handlePageClick = async (e) => {
    ...
    await BrandListRequest(pageNo + 1, perPage, searchKeyword)
  }
  ...
  const DeleteItem = async (id) => {
    ...
    if (res.isConfirmed) {
      let DeleteResult = await BrandDeleteRequest(id)
      if (DeleteResult) {
        await BrandListRequest(1, perPage, searchKeyword)
      }
    }
                              ...
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    ...
                                    <td>
                                      <Link to={`/BrandCreateUpdatePage?id=${item._id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm"> <AiOutlineEdit size={15} /></Link>
