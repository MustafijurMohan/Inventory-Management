...
import { ExpenseTypeDeleteRequest, ExpenseTypeListRequest } from '../../api/ExpenseTypeApiRequest';
...
const ExpenseTypeList = () => {
  ...
  const handlePageClick = async (e) => {
    ...
    await ExpenseTypeListRequest(pageNo + 1, perPage, searchKeyword)
  }
  ...
  const TextSearch = (e) => {
    ...
    const DeleteItem = async (id) => {
      ...
      if (res.isConfirmed) {
        let DeleteResult = await ExpenseTypeDeleteRequest(id)
        if (DeleteResult) {
          await ExpenseTypeListRequest(1, perPage, searchKeyword)
        }
      }
                              ...
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    ...
                                    <td>
                                      <Link to={`/ExpenseTypeCreateUpdatePage?id=${item._id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm"> <AiOutlineEdit size={15} /></Link>
