...
import { CategoryDeleteRequest, CategoryListRequest } from '../../api/CategoryApiRequest';
...
const CategoryList = () => {
  ...
  const DataList = useSelector((state) => state.category.List)
  const Total = useSelector((state) => state.category.ListTotal)


  const handlePageClick = async (e) => {
    let pageNo = e.selected
    await CategoryListRequest(pageNo + 1, perPage, searchKeyword)
  }
  ...
  const TextSearch = (e) => {
    ...
    const DeleteItem = async (id) => {
      ...
      if (res.isConfirmed) {
        let DeleteResult = await CategoryDeleteRequest(id)
        if (DeleteResult) {
          await CategoryListRequest(1, perPage, searchKeyword)
        }
      }
                              ...
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    ...
                                    <td>
                                      <Link to={`/CategoryCreateUpdatePage?id=${item._id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm"> <AiOutlineEdit size={15} /></Link>
