...
import { SupplierDeleteRequest, SupplierListRequest } from '../../api/SupplierApiRequest';
...
const SupplierList = () => {
  ...
  const DataList = useSelector((state) => state.supplier.List)
  const Total = useSelector((state) => state.supplier.ListTotal)


  const handlePageClick = async (e) => {
    let pageNo = e.selected
    await SupplierListRequest(pageNo + 1, perPage, searchKeyword)
  }
...
const DeleteItem = async (id) => {
  ...
  if (res.isConfirmed) {
    let DeleteResult = await SupplierDeleteRequest(id)
    if (DeleteResult) {
      await SupplierListRequest(1, perPage, searchKeyword)
    }
  }
                              ...
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    ...
                                    <td>
                                      <Link to={`/SupplierCreateUpdatePage?id=${item._id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm"> <AiOutlineEdit size={15} /></Link>
