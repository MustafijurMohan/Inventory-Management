...
import { ExpenseDeleteRequest, ExpenseListRequest } from '../../api/ExpenseApiRequest';
...
const ExpenseList = () => {
  ...
  const handlePageClick = async (e) => {
    ...
    await ExpenseListRequest(pageNo + 1, perPage, searchKeyword)
  }
...
const DeleteItem = async (id) => {
  ...
  if (res.isConfirmed) {
    let DeleteResult = await ExpenseDeleteRequest(id)
    if (DeleteResult) {
      await ExpenseListRequest(1, perPage, searchKeyword)
    }
  }
                              ...
                              DataList.map((item, i) => {
                                return (
                                  <tr key={i.toString()}>
                                    ...
                                    <td>
                                      <Link to={`/ExpenseCreateUpdatePage?id=${item._id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm"> <AiOutlineEdit size={15} /></Link>
